/*------------------------------------------------------------------------
  Built from Adafruit's Arduino library for the ESP8266 WiFi-serial bridge
...
void ESP8266::setBootMarker(Fstr *s) {
...
boolean ESP8266::find(Fstr *str, boolean ipd) {
...
int ESP8266::readLine(char *buf, int bufSiz) {
  if(debug && writing) {
    debug->print(F("<--- '"));
    writing = false;
  }
...
// false otherwise.
boolean ESP8266::hardReset(void) {
...
boolean ESP8266::softReset(void) {
  boolean  found = false;
...
boolean ESP8266::connectToAP(Fstr *ssid, Fstr *pass) {
  char buf[256];
  ...
  if(!(strstr_P(buf, (Pchr *)F("OK")) ||
    strstr_P(buf, (Pchr *)F("no change")))) {
         /* Getting extraneous CRLF, fetch next line in case... */
    readLine(buf, sizeof(buf));
    if(!(strstr_P(buf, (Pchr *)F("OK")) ||
      strstr_P(buf, (Pchr *)F("no change")))) {
        return false;
...
boolean ESP8266::connectTCP(Fstr *h, int port) {
...
boolean ESP8266::requestURL(Fstr *url) {
  print(F("AT+CIPSEND="));
...
// Posts to currently-open TCP connection.
boolean ESP8266::postURL(Fstr *url) {
  print(F("AT+CIPSEND="));
  println(26 + strlen_P((Pchr *)url) + strlen_P((Pchr *)host));
  if(find(F("> "))) { // Wait for prompt
    print(F("POST ")); // 5
    print(url);
    print(F(" HTTP/1.1\r\nHost: ")); // 17
...
// need to parse IPD delimiters (see notes in find() function.
// (Can call find(F("Unlink"), true) to dump to debug.)
boolean ESP8266::requestURL(char* url) {
    print(F("AT+CIPSEND="));
